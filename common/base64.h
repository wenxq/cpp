 ...
 *   at least base64_encode_len(len) bytes (see below)
 ...
 * char* dest = (char*) malloc(base64_encode_len);
 * int len = base64_encode(dest, src, sourcelen);
 * if (len == -1) {
 *   printf("Error\n");
 * } else {
 *   printf("b64 = %s\n", dest);
 ...
 * char* dest = (char*) malloc(base64_decode_len(srclen));
 * int len = base64_decode(dest, src, sourcelen);
 * if (len == -1) { error }
 * \endcode
 */
 ...
 * if (base64_encode_strlen(sizeof(struct datastuff)) != len) {
 *    // wrong size
 *    return false;
 * } else {
 *    // safe to do;
 *    if (base64_decode((char*) &foo, b64encoded, len) == -1) {
 *      // bad characters
 *      return false;
 *    }
 * }
...
inline std::string& base64_encode(std::string& s)
{
    ...
    int d = base64_encode(const_cast<char*>(x.data()), s.data(), integer_cast<int>(s.size()));
...
inline std::string& base64_decode(std::string& s)
{
    ...
    int d = base64_decode(const_cast<char*>(x.data()), s.data(), integer_cast<int>(s.size()));
    if (d < 0) {
        x.clear();
    } else {
        x.erase(d, std::string::npos);
