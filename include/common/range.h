 ...
 * with the former Slice class, which is used by a lot of other
...
public:
    template <size_t N>
    Range(value_type (&buffer)[N])
        : b_(buffer), e_(buffer + N)
    {}
    ...
    // Allow implicit conversion from Range<const char*> (aka Slice) to
     ...
     */
    bool starts_with(const Range& other) const
    {
     ...
     */
    bool ends_with(const Range& other) const
    {
     ...
     */
    bool remove_prefix(const Range& prefix)
    {
        return starts_with(prefix) && (b_ += prefix.size(), true);
    }
    bool remove_prefix(value_type prefix)
    {
        return starts_with(prefix) && (++b_, true);
    }
     ...
     */
    bool remove_suffix(const Range& suffix)
    {
        return endsWith(suffix) && (e_ -= suffix.size(), true);
    }
    bool remove_suffix(value_type suffix)
    {
        return ends_with(suffix) && (--e_, true);
    }
     ...
     * Example:
     *
     *  Slice s("sample string for split_next");
     *  auto p = s.split_step(' ');
     *
     *  // prints "sample"
     *  cout << s << endl;
     ...
     * Example 2:
     *
     *  void tokenize(Slice s, char delimiter) {
     *    while (!s.empty()) {
...
typedef Range<char*> MutableSlice;
